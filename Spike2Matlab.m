%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This function will detect all .smr or .smrx files in the current folder
%and convert them into .mat files (will also DELETE the original data!!!).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(spike2List)
    convertSpike2(spike2List{i});
end

delete('RawSpike2files.txt');
CEDS64CloseAll();

%% Function to convert a certain spike2 file to .mat file
...
%}
    ...
    for ichan = 1:counter %only loop through the used channel
        ...
        if ~isempty(fVals)
            ...
            %If the sampling rate is lower than 1000Hz, do not downsample.
            if skipIntvl < 1
                skipIntvl = 1;
            end
